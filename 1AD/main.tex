...
We begin by calculating $f(z^{(s)}) = \log p(X, z^{(s)}) - \log q(z^{(s)}|\Theta)$ for each sample $s$.
Next, we compute the score function for each sample $s$ and each variational parameter $\theta_j$.
Then, we calculate the optimal baselines ($a^*$) (control variates) for each parameter and set the gradient estimate using the baseline ($a^*$).
With these modifications, we run the BBVI algorithm and obtain the following plots for dataset 2 with 100 samples.
The following results were obtained:
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{Plots/ELBO_convergence_dataset2_control_variates.png}
  \caption{ELBO over iterations for dataset with 100 samples using BBVI with control variates.}
\end{figure}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\textwidth]{Plots/expected_mu_tau_convergence_control_variates.png }
  \caption{Expected value of $\mu$ and $\tau$ over iterations for dataset with 100 samples using BBVI with control variates.}
\end{figure}
\subsection*{Practice/implementation - 1.C.4}
\subsubsection*{Question 2.2.14}
The gamma distribution pdf is defined as
\begin{equation*}
  p(x|\alpha, \beta) = \frac{x^{\alpha-1}\beta^\alpha}{\Gamma(\alpha)}\text{exp}(-\beta x) = \text{exp}((\alpha-1)log(x) - \beta x - log(\Gamma(\alpha)) + \alpha log(\beta))
\end{equation*}
In, canonical exponential form we can use the following identifications:
\begin{itemize}
    \item $\boldsymbol{\eta}(\boldsymbol{\theta}) = [\alpha - 1, -\beta]$
    \item $h(x) = 1$
    \item $\boldsymbol{T}(x) = [\log x, x]$
    \item $A(\boldsymbol{\eta}) = \log \Gamma(\eta_1 + 1) - (\eta_1 + 1)\log(-\eta_2)$
\end{itemize}
...
Then we have that the natural gradient is given by:
\begin{equation*}
...
Furthermore, natural gradient descent does not reach a good approximation, even after 150 epochs.
...
as a deterministic function of a random variable $\epsilon$, with some fixed sitribution and the parameters $\phi$
...
\end{figure}
We can clearly see that the Stop-Gradient VAE outperforms the standard VAE implementation from 2E.
It is much more stable, converges faster and to a better ELBO value.
\section*{A-level}
\subsection*{1.A.1 - Theory}
\subsubsection*{Question 4.1.26}
\subsection*{1.A.2 - Practice/implementation}
